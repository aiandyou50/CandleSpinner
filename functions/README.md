***REMOVED***⚙️ Backend - Cloudflare Workers (functions/)
...
├── _bufferPolyfill.ts       ***REMOVED***Buffer 폴리필 (Node.js 호환성)
├── _rateLimit.ts            ***REMOVED***Rate Limiting 구현
├── _headers                 ***REMOVED***HTTP 헤더 설정
│
└── 📁 api/                  ***REMOVED***API 엔드포인트
    ├── RATE_LIMITING_GUIDE.md
    │
    ├── 💰 입금 (Deposit)
    │   ├── initiate-deposit.ts      ***REMOVED***✅ 입금 시작 (V5R1, SendMode 적용)
    │   ├── deposit.ts               ***REMOVED***입금 완료 처리
    │   ├── deposit-rpc.ts           ***REMOVED***RPC 호출
    │   └── credit-deposit.ts        ***REMOVED***크레딧 적용
    │
    ├── 💸 인출 (Withdrawal)
    │   ├── initiate-withdrawal.ts   ***REMOVED***✅ 인출 시작 (V5R1, SendMode 적용)
    │   ├── debug-withdrawal.ts      ***REMOVED***인출 디버그
    │   ├── get-withdrawal-logs.ts   ***REMOVED***인출 기록 조회
    │   └── double-up.ts             ***REMOVED***배팅 가능 금액 계산
    │
    ├── 🎮 게임 로직 (Game Logic)
    │   ├── spin.ts                  ***REMOVED***스핀 결과 계산 (오프체인)
    │   ├── collect-winnings.ts      ***REMOVED***상금 수집
    │   ├── save-game-state.ts       ***REMOVED***게임 상태 저장
    │   └── get-credit.ts            ***REMOVED***크레딧 조회
    │
    ├── 🔐 보안 & 검증 (Security)
    │   ├── generate-wallet.ts       ***REMOVED***지갑 생성 (테스트용, V5R1)
    │   ├── debug-private-key.ts     ***REMOVED***✅ 개인키 검증 (V5R1)
    │   └── check-developer-password.ts ***REMOVED***개발자 모드
    │
    └── 📝 로깅 & 모니터링
        └── (Sentry 통합)
```
