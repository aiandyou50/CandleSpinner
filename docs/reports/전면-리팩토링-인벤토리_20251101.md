...
#***REMOVED***3. 디렉터리 재구성 제안 (초안)

##***REMOVED***3.1 프론트엔드 (`src/`)

```
src/
  app/              ***REMOVED***진입점 및 전역 설정 (App.tsx, main.tsx)
  features/
    game/
    wallet/
    analytics/
  shared/
    components/
    hooks/
    lib/            ***REMOVED***API 래퍼, logger 등
    ui/             ***REMOVED***공용 UI (DebugPanel 포함)
```

- Legacy 컴포넌트는 `archive/legacy-code/frontend/`로 이동하여 README로 배경 기록.

##***REMOVED***3.2 백엔드 Functions (`functions/`)

```
functions/
  index.ts
  src/
    handlers/       ***REMOVED***Cloudflare HTTP 핸들러 (initiate-withdrawal 등)
    services/       ***REMOVED***비즈니스 로직 (Ton RPC, credit 관리)
    lib/            ***REMOVED***공용 유틸 (mnemonic, rpc, rateLimit 등)
    types/
  archive/
    legacy/         ***REMOVED***과거 버전 핸들러/백업 코드
```

- 현재 `functions/api/*.ts` 구조를 단계적으로 위 구조로 이동.

##***REMOVED***3.3 아카이브/레거시 분리
...
#***REMOVED***4. RPC Seqno 오류 조사 로깅 계획
