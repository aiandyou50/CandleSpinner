...
##***REMOVED***가능한 원인 (우선순위순)

###***REMOVED***1️⃣ **환경변수 누락** (가장 높은 확률)
```typescript
...
###***REMOVED***3️⃣ **TonAPI 네트워크 오류** (중간 확률)
```typescript
// Line 88-102
...
##***REMOVED***2. wrangler.toml KV 바인딩 확인
```
...
##***REMOVED***3. Cloudflare Worker 로그 확인
```
...
#***REMOVED***🔧 즉시 확인할 체크리스트

##***REMOVED***Step 1: 환경변수 확인
```
...
#***REMOVED***📱 임시 해결 방법 (테스트용)

만약 환경변수 문제라면, 로컬 개발 환경에서 테스트:

```bash
***REMOVED***Step 1: 로컬 환경변수 설정
cp .env.example .env.local

***REMOVED***Step 2: wrangler.toml에 로컬 값 추가 (개발용만)
cat >> wrangler.toml << 'EOF'
[env.development]
name = "candlespinner-dev"
EOF

***REMOVED***Step 3: 로컬 서버 시작
npm run dev

***REMOVED***Step 4: 로컬에서 인출 테스트
