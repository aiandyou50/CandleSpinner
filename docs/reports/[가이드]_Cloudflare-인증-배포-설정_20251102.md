...
### 개발 모드
```bash
npm run dev          # Vite 로컬 서버
npx wrangler dev     # Workers 로컬 서버
```
...
npm run deploy  # package.json에 "deploy" 스크립트 필요
```
...
npm run deploy:watch    # 파일 변경 감지 + 자동 배포
```
...
- **잘못된 명령:**
  ```bash
  npx wrangler pages deploy dist  # ❌ Pages 명령 (사용 금지)
  ```
- **올바른 명령:**
  ```bash
  npx wrangler deploy  # ✅ Workers 명령
  ```
...
### C. wrangler.toml 설정
```toml
name = "candlespinner-workers"  # ✅ 프로젝트 이름 (변경 금지)
compatibility_date = "2024-11-01"
main = "src/index.ts"
node_compat = true

[assets]
directory = "./dist"  # ✅ Vite 빌드 결과물
...
### 문제: "Authentication required"
```bash
npx wrangler logout
npx wrangler login
```

### 문제: "KV namespace not found"
```bash
# wrangler.toml에서 KV ID 확인
