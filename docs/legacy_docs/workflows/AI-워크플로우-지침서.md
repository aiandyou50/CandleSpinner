...
[Step 1] 요청 분석 및 문서 확인 (Audit-Before-Action) + 🔐 보안 검증
 * **보안 검증 (필수 - 절대 생략 금지)**
   * 요청에서 민감한 정보(개인키, 니모닉, API 토큰)가 포함되었는가? → 즉시 경고
   * 코드에서 하드코딩된 개인키가 있는가? → `grep "4e6568\|tornado run"` 검사
   * wrangler.toml에 민감한 정보가 있는가? → 커밋 거부 및 수정 지시
   * **위반 시:** workflows/[보안-정책]_보안-워크플로우-강제.md 참조 후 수정 실시
 ...
 * 사용자에게 다음과 같이 확인 응답합니다.
   ...
   * (보안 위반 시): "🔐 보안 정책 위반 감지. [Step 1]을 다시 수행합니다..."
