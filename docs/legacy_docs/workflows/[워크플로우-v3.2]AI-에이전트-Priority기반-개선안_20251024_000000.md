***REMOVED***[워크플로우 v3.2] AI 에이전트 개선된 작업 흐름 (Priority 기반)
...
```markdown
##***REMOVED***Step 1: [SKIP] 문서 확인
- 이미 문서화된 범위 내이므로 생략

##***REMOVED***Step 2: [SKIP] 명령 아카이빙
- 임시 요청이므로 생략

##***REMOVED***Step 3: 코드 수정 (필수)
- [ ] 버그/기능 수정 실행
- [ ] 테스트 완료

##***REMOVED***Step 4: CHANGELOG 간단 기록 (필수 - 1-2줄)
- [ ] CHANGELOG.md에 "[tag] 간단한 설명" 추가
- [ ] 예: "[fix] 사용자 입금 validation 오류 수정"
- [ ] 버전 업데이트: PATCH 레벨만 (+0.0.1)
- [ ] package.json 동기화

##***REMOVED***Step 5: [SKIP] 해결 기록
- 중요도 낮으므로 생략
...
```markdown
##***REMOVED***Step 1-5: [모두 SKIP]
- 빠른 처리를 위해 모두 생략

##***REMOVED***Step 6: 칸반 업데이트 (필수만)
- [ ] kanban.md 업데이트 → [Done]
...
#***REMOVED***📋 각 Priority별 실행 체크리스트

##***REMOVED***Priority A 체크리스트
```markdown
###***REMOVED***[Priority A] 아키텍처 변경 - Full 워크플로우
...
##***REMOVED***Priority B 체크리스트
```markdown
###***REMOVED***[Priority B] 기능/버그 - Mid 워크플로우
...
##***REMOVED***Priority C 체크리스트
```markdown
###***REMOVED***[Priority C] 문서/리팩토링 - Light 워크플로우
...
#***REMOVED***🎯 자동화 규칙 (파일별 Priority 결정)
