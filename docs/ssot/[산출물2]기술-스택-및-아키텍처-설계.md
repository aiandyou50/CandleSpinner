***REMOVED***[산출물 2] 기술 스택 및 아키텍처 (v2.1.0 A/B Test Edition)

**프로젝트명**: CandleSpinner  
**버전**: 2.1.0  
**마지막 업데이트**: 2025년 10월 21일  
**상태**: MVP + A/B 테스트 준비 (v2.1.0 스프린트 진행 중)
 ...
 * 지갑 연결 UI: @tonconnect/ui-react
   ...
   * **v2.1.0 개선사항**: TMA 환경과 웹 환경 모두에서 TonConnect 버튼을 표시합니다. (이전: 웹 환경에서만 표시)
     - `App.tsx`: TMA 환경 `<header>`에 `<TonConnectButton />` 추가
     - 입금 화면에서 모든 환경에서 지갑 연결 가능
