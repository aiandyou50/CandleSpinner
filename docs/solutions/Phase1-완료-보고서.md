***REMOVED***Phase 1 완료 보고서: RPC 인출 오류 해결
...
###***REMOVED***1. Cloudflare Pages 환경변수 설정
...
```bash
***REMOVED***Option 1: 로컬 스크립트 실행
node wallet-tools/mnemonic-to-key.mjs "your 24 word mnemonic here"

***REMOVED***출력 확인:
***REMOVED***- Wallet Address (UQ...)가 GAME_WALLET_ADDRESS와 일치하는지 확인
```

##***REMOVED***배포 절차

###***REMOVED***Step 1: PR 머지
```bash
***REMOVED***GitHub에서 PR 승인 및 머지
***REMOVED***브랜치: copilot/fix-bad-secret-key-error → main
```

###***REMOVED***Step 2: Cloudflare Pages 자동 배포
- PR 머지 시 자동으로 배포 시작
- 배포 상태: https://dash.cloudflare.com/[account]/pages/candlespinner/deployments

###***REMOVED***Step 3: 배포 확인
배포 완료 후 약 2-3분 대기
...
#***REMOVED***🔍 문제 해결 (Troubleshooting)

##***REMOVED***문제 1: "유효하지 않은 니모닉: BIP39 검증 실패"
...
1. 니모닉으로부터 실제 주소 생성:
   ```bash
   node wallet-tools/mnemonic-to-key.mjs "your mnemonic"
   ***REMOVED***출력된 Non-Bounceable (UQ...) 주소를 GAME_WALLET_ADDRESS에 설정
   ```

2. 또는: 올바른 니모닉 사용 (해당 주소와 매칭되는 니모닉)

##***REMOVED***문제 3: "게임 지갑 TON 부족"

**원인**: 인출 트랜잭션 수수료를 위한 TON 잔액 부족

**해결 방법**:
```bash
***REMOVED***게임 지갑에 최소 0.1 TON 충전
***REMOVED***주소: UQBFPDdSlPgqPrn2XwhpVq0KQExN2kv83_batQ-dptaR8Mtd
```
