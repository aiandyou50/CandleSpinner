***REMOVED***[해결완료] 전체 디렉토리 정리 및 구조화 (2025-10-26)
...
###***REMOVED***✅ docs/reports로 이동한 JSON 파일 (3개)
| 파일명 | 목적지 |
|------|--------|
...
##***REMOVED***2️⃣ 루트 폴더 정리

###***REMOVED***✅ docs/workflows로 이동한 파일 (1개)
| 파일명 | 목적지 |
|------|--------|
| `COMMIT_MESSAGE.md` | `docs/workflows/` |

##***REMOVED***3️⃣ Docs 폴더 정리

###***REMOVED***✅ docs/solutions로 이동한 문서 (5개)
| 파일명 | 목적지 |
|------|--------|
...
##***REMOVED***4️⃣ Blueprint 배포 스크립트 수정

###***REMOVED***✅ blueprintDeploy.ts 오류 수정
...
CandleSpinner/
├── docs/
│   ├── ssot/                    ***REMOVED***SSOT 문서 (프로젝트 기준)
│   ├── instructions/            ***REMOVED***사용자 지시 기록
│   ├── workflows/               ***REMOVED***워크플로우 지침 + COMMIT_MESSAGE.md
│   ├── solutions/               ***REMOVED***해결 기록 + 배포 가이드
│   ├── reports/                 ***REMOVED***임시 보고서 + deployment JSON
│   ├── 레거시/
│   │   ├── deployment-guides/   ***REMOVED***오래된 배포 문서
│   │   ├── reports/
│   │   ├── solutions/
│   │   ├── ssot/
│   │   └── workflows/
...
##***REMOVED***blueprintDeploy.ts 변경 사항
...
```bash
***REMOVED***방법 1: TonConnect 사용 (권장)
cd contracts
npx blueprint run deployWithTonConnect --testnet

***REMOVED***방법 2: Mnemonic 직접 (환경 변수 필요)
