***REMOVED***PoC 복원 가이드
#***REMOVED***CandleSpinner PoC 코드 복원 방법
...
###***REMOVED***방법 1: 터미널 명령어로 복원 (권장)
```bash
***REMOVED***1. 프로젝트 루트로 이동
cd "c:\Users\x0051\Desktop\DEV\새 폴더\CandleSpinner"

***REMOVED***2. src 폴더 복원
cp -r PoC/src/* src/

***REMOVED***3. functions 폴더 생성 및 복원
mkdir -p functions
cp -r PoC/functions/* functions/

***REMOVED***4. docs 폴더 생성 및 복원
mkdir -p docs
cp -r PoC/docs/* docs/

***REMOVED***5. 의존성 설치 및 실행
npm install
npm run dev
```

###***REMOVED***방법 2: 수동 파일 복사
1. 파일 탐색기로 `PoC/` 폴더 열기
...
###***REMOVED***방법 3: 백업 ZIP 파일 사용 (가장 안전)
...
###***REMOVED***1. 코드 주석 (App.tsx)
`src/App.tsx` 파일에 복원 방법이 주석으로 기록되어 있습니다.

###***REMOVED***2. 이 문서 (docs/PoC-restoration-guide.md)
...
###***REMOVED***복원 후 오류가 발생하는 경우
```bash
***REMOVED***1. 캐시 정리
npm run clean  ***REMOVED***또는 수동으로 node_modules 삭제 후 재설치

***REMOVED***2. TypeScript 캐시 재시작 (VS Code 명령어)
***REMOVED***Ctrl+Shift+P → "TypeScript: Restart TS Server"
