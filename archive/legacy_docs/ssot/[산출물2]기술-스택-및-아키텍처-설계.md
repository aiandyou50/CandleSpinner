# [산출물 2] 기술 스택 및 아키텍처 설계 (2025-11-02)
...
└───────────────┬──────────────┘
                ...
                ▼
┌────────────────────────────────────┐
│ React SPA (Vite 빌드 산출물)       │
│ - TonConnect UI                    │
│ - SlotMachineV2 / HelpDialog       │
...
└───────────────┬────────────────────┘
                ...
                ▼
┌────────────────┐   ┌────────────────────┐
│ Cloudflare KV  │   │ TON Network        │
│ - Credit 상태  │   │ - CSPIN Jetton     │
│ - Game 로그    │   │ - TonConnect 지갑  │
│ - Withdraw 큐  │   │ - 0.2 TON 수수료   │
└────────────────┘   └────────────────────┘
```
...
| 항목 | 버전 | 용도 |
|------|------|------|
| React | 18.3.1 | SPA UI 구성 |
| TypeScript | 5.6.3 | 정적 타입 |
| Vite | 5.4.10 | 번들링/개발 서버 |
| MUI | 7.3.5 | 레이아웃/컴포넌트 |
| Tailwind CSS | 3.4.14 | 유틸리티 스타일 |
| Framer Motion | 12.23.24 | 애니메이션 |
| @tonconnect/ui-react | 2.0.9 | TonConnect 버튼/세션 |
| i18next + react-i18next | 25.x / 16.x | 다국어 지원 |

### 2.2 백엔드 (Worker)

| 항목 | 버전 | 설명 |
|------|------|------|
| Cloudflare Workers | 2025.10 릴리스 | 서버리스 런타임 |
| Wrangler | 3.82.0 | 배포/로컬 실행 |
| @cloudflare/workers-types | 4.20251014.0 | 타입 정의 |
| @ton/ton, @ton/crypto | 15.0.0 / 3.3.0 | Jetton 주소 조회, BOC 검증 |

### 2.3 기타

- TonCenter JSON-RPC (`https://toncenter.com/api/v2/`)  
- TonConnect manifest (`public/tonconnect-manifest.json`)  
- Cloudflare KV (Wrangler `[[kv_namespaces]]` 바인딩 `CREDIT_KV`)

## 3. 주요 컴포넌트 매핑

| 영역 | 파일 | 설명 |
|------|------|------|
| 앱 엔트리 | `src/App.tsx` | 라우팅, AppBar, Drawer, HelpDialog 제어 |
| 지갑 훅 | `src/hooks/useTonConnect.ts` | TonConnect 상태/주소 제공 |
| 크레딧 훅 | `src/hooks/useCredit.ts` | `GET /api/credit` 호출, 실시간 갱신 |
| 입금 UI | `src/components/Deposit.tsx` | TonConnect Jetton Transfer + `/api/verify-deposit` |
| 인출 UI | `src/components/Withdraw.tsx` | 0.2 TON 수수료 전송 + `/api/withdraw-request` |
| 슬롯 UI | `src/features/slot/components/SlotMachineV2.tsx` | 스핀 버튼, 결과 표시, 더블업 트리거 |
| 언어 선택 | `src/components/LanguageSelector.tsx` | 9개 언어 드롭다운 |
| Worker 엔트리 | `src/index.ts` | API 라우터, CORS, Static 서빙 |
| 슬롯 핸들러 | `functions/src/slot/spin-handler.ts` | Provably Fair 스핀, RTP 갱신 |
| 더블업 | `functions/src/slot/doubleup-handler.ts` | 50% 승패, RTP 반영 |
| 페이아웃 | `functions/src/slot/payout-calculator.ts` | 배당 계산, 잭팟 로직 |
| Withdraw Queue | `src/index.ts` (handleWithdrawRequest) | 논스 검증, KV 큐 저장 |
| Admin UI | `src/app/AdminWithdrawals.tsx` | 대기열 조회, 처리 표시 |

## 4. API 계약

| 경로 | 메서드 | 요청 주요 필드 | 응답 요약 |
|------|--------|----------------|-----------|
| `/api/credit` | GET | `address` (query) | `{ success, credit, lastUpdated }` |
| `/api/check-balance` | POST | `{ jettonWalletAddress }` | TonCenter proxy 결과 (잔액 또는 에러) |
| `/api/verify-deposit` | POST | `{ walletAddress, txHash, amount }` | `{ success, credit }` (검증 실패 시 에러 메시지) |
| `/api/slot/spin` | POST | `{ walletAddress, betAmount, clientSeed }` | `{ success, result, winAmount, newCredit, serverSeedHash, gameId }` |
| `/api/slot/doubleup` | POST | `{ walletAddress, choice, currentWin, gameId }` | `{ success, result: 'win'|'lose', newCredit }` |
| `/api/slot/rtp-stats` | GET | - | `{ success, stats: { totalGames, totalBets, totalWins, rtp } }` |
| `/api/withdraw-request` | POST | `{ action, amount, userAddress, timestamp, nonce, feeTxBoc?, feeTonAmount?, feePaidAt? }` | `{ success, withdrawalId, credit, estimatedProcessTime }` |
| `/api/admin/pending-withdrawals` | GET | - | `{ success, withdrawals: WithdrawalRecord[] }` |
| `/api/admin/mark-processed` | POST | `{ withdrawalId, txHash? }` | `{ success }` |

> 참고: `/api/spin`, `/api/withdraw` 등 레거시 엔드포인트는 하위 호환용으로 남아 있으나 현재 UI에서는 호출하지 않는다.

## 5. Cloudflare KV 스키마

| 키 패턴 | 값 형태 | 보존 기간 |
|---------|---------|-----------|
| `credit:<wallet>` | `{ credit: number, lastUpdated: ISOString }` | 기본 (무기한) |
| `server_seed:<wallet>` | `string` (비공개 시드) | 무기한 |
| `nonce:<wallet>` | `string` (int) | 무기한 |
| `game:<id>` | 슬롯 결과 JSON (`betAmount`, `reelResults`, `totalWin`, `timestamp`, ...) | 30일 |
| `doubleup_used:<gameId>` | `'true'` | 10분 |
| `rtp_stats:<YYYY-MM-DD>` | `{ totalGames, totalBets, totalWins, rtp }` | 90일 |
| `withdrawal:<id>` | `{ walletAddress, amount, status, requestedAt, processedAt?, txHash? }` | 60일 |
| `withdrawals:pending` | `string[]` (ID 리스트) | 무기한 |
| `nonce:<value>` | `'used'` (리플레이 방지) | 10분 |

## 6. 시퀀스 다이어그램 (텍스트)

### 6.1 입금

1. 사용자 TonConnect 연결 → `Deposit.tsx`에서 금액 입력.  
2. Jetton Wallet 주소 계산 (`JettonMaster.getWalletAddress`).  
3. TonConnect 트랜잭션 생성, 0.05 TON 가스 포함.  
4. 지갑이 BOC 반환 → `/api/verify-deposit` POST.  
5. Worker가 TonCenter를 통해 BOC 검증, 크레딧 업데이트.  
6. 프론트가 `useCredit`를 통해 갱신된 크레딧 표시.

### 6.2 슬롯 스핀

1. UI가 `/api/slot/spin` 호출 (betAmount 10~1000, clientSeed 포함).  
2. Worker가 서버 시드/논스 결합 → 3x3 결과 생성.  
3. `calculatePayout`로 중앙 라인 2개 이상 일치 심볼만 집계.  
4. 잭팟 시 총합 x100, 리턴 JSON에 `isJackpot: true`.  
5. 결과 저장 (`game:<id>`), RTP 갱신, 응답 반환.  
6. UI에서 애니메이션 플레이, 더블업 모달 노출 조건 판단.

### 6.3 인출 요청

1. 사용자가 0.2 TON 수수료 트랜잭션을 TonConnect로 서명.  
2. UI가 수수료 BOC 및 타임스탬프를 저장 (10분 유효).  
3. `/api/withdraw-request`에 `nonce`, `timestamp`, `amount`와 함께 제출.  
4. Worker는 타임스탬프, nonce, 크레딧 검증 후 즉시 차감.  
5. `withdrawal:<id>` 기록 생성, `withdrawals:pending` 큐에 push.  
6. 응답으로 `withdrawalId`, `estimatedProcessTime` 제공.  
7. 운영자가 `/admin`에서 확인 후 수동 송금 → `mark-processed` 호출로 상태 업데이트.

## 7. 운영 및 배포

- **로컬 개발:** `npm run dev` + `npm run wrangler:dev` 병행 실행.  
- **배포:** `npm run deploy` 또는 `npm run wrangler:deploy` (Vite 빌드 후 Worker 배포).  
- **환경 변수:** `wrangler.toml`에 KV 바인딩과 비밀 키 지정 (`wrangler secret put`).  
- **모니터링:** Cloudflare Worker 로그 + 브라우저 콘솔(`logger` 유틸). TonCenter rate limit 에러 시 경고 로그 확인.  
- **백업:** 중요한 KV 키(`credit:*`, `withdrawal:*`)는 주기적으로 export (`wrangler kv export`) 권장.

---

**Revision:** 2025-11-02 / 작성자: GitHub Copilot (aiandyou50)
***REMOVED***[산출물 2] 기술 스택 및 아키텍처 (v2.3.0)
...
##***REMOVED***1.1 전체 흐름도 (RPC 직접 인출 방식)
...
| 컴포넌트 | 파일 | 역할 |
|---------|------|------|
| **App.tsx** | src/app/App.tsx | 앱 엔트리, 게임/입금 모드 전환 |
| **GameComplete.tsx** | src/features/game/GameComplete.tsx | 메인 게임 (스핀/출금) |
| **ReelPixi.tsx** | src/features/game/ReelPixi.tsx | 3릴 애니메이션 |
| **Deposit.tsx** | src/features/deposit/Deposit.tsx | TonConnect & RPC 입금 |
| **DebugPanel.tsx** | src/shared/ui/DebugPanel.tsx | 프론트엔드 디버그 로그 패널 |
| **ErrorBoundary.tsx** | src/shared/components/ErrorBoundary.tsx | 전역 오류 경계 |
...
#***REMOVED***5.1 `/api/initiate-withdrawal` 상세 (v2.3.0 - RPC 방식)
