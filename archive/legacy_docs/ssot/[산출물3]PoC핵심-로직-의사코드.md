# [산출물 3] PoC 핵심 로직 의사코드 (2025-11-02)

PoC 범위는 “TonConnect를 통해 사용자가 CSPIN을 게임 지갑으로 전송하고, Worker가 입금을 확인해 크레딧을 적립한다”이다. 아래 의사코드는 현재 코드베이스(`src/components/Deposit.tsx`, `/api/verify-deposit`)를 단순화한 버전이다.

## 1. 전역 설정

```pseudocode
CONSTANT GAME_WALLET_ADDRESS = "UQBFPDdSlPgqPrn2XwhpVq0KQExN2kv83_batQ-dptaR8Mtd"
CONSTANT CSPIN_TOKEN_ADDRESS = "EQBZ6nHfmT2wct9d4MoOdNPzhtUGXOds1y3NTmYUFHAA3uvV"
CONSTANT GAME_JETTON_WALLET = "EQAjtIvLT_y9GNBAikrD7ThH3f4BI-h_l_mz-Bhuc4_c7wOs"
CONSTANT TON_CONNECT_MANIFEST_URL = "https://aiandyou.me/tonconnect-manifest.json"
```
...
```pseudocode
STATE amount = "10"
HOOK wallet = useTonWallet()
HOOK tonConnectUI = useTonConnectUI()

FUNCTION handleDeposit()
    REQUIRE wallet IS NOT null

    userJettonWallet = TonCenter.getWalletAddress(
        master = CSPIN_TOKEN_ADDRESS,
        owner = wallet.address
    )

    payload = buildJettonTransferPayload(
        amount = toNano(amount),
        destination = GAME_WALLET_ADDRESS,
        responseTo = wallet.address
    )

    transaction = {
        validUntil: now() + 300 seconds,
        messages: [{
    ...
    result = tonConnectUI.sendTransaction(transaction)
...
```pseudocode
FUNCTION handleVerifyDeposit(request)
    body = parseJSON(request)
    REQUIRE body.amount > 0

    isValid = TonCenter.verifyJettonTransfer(
        boc = body.txHash,
        from = body.walletAddress,
        toJettonWallet = GAME_JETTON_WALLET,
        amount = body.amount
    )
    ...
    credit = KV.getCredit(body.walletAddress) + body.amount
    KV.setCredit(body.walletAddress, credit)
...
- 사용자가 TonConnect 팝업에서 트랜잭션을 서명하면 0.05 TON 수수료로 CSPIN이 게임 지갑에 도달한다.  
- Worker가 BOC를 확인해 크레딧을 즉시 업데이트한다.  
- `useCredit` 훅 혹은 `/api/credit` 호출로 UI에서 새로운 크레딧을 확인할 수 있다.

---

**주요 차이:** PoC는 인출, 슬롯, RTP 통계, 더블업과 같은 고급 기능을 포함하지 않으며, Fee 검증이나 관리자 큐도 다루지 않는다.
