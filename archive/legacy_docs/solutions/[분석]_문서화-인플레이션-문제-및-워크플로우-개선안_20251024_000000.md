***REMOVED***[분석] 문서화 인플레이션 문제 및 워크플로우 개선안
...
docs/
│
├── 📌 ssot/                      ***REMOVED***[핵심 SSoT] - 절대 변함없음
│   ├── README.md                 ***REMOVED***⭐ 마스터 참고서 (모든 정보 중앙화)
│   ├── [산출물1]프로젝트-정의서.md
│   ├── [산출물2]기술-스택-및-아키텍처-설계.md
│   ├── [산출물3]MVP핵심-로직-의사코드.md
│   ├── [통합-변경-로그].md        ***REMOVED***✅ NEW: CHANGELOG.md와 동기화
│   └── INDEX.md                  ***REMOVED***전체 맵
│
├── 📋 workflows/                 ***REMOVED***[워크플로우] - 2개 유지
│   ├── AI-워크플로우-지침서.md    ***REMOVED***Full (상세, Reference용)
│   ├── 퀵-AI워크플로워-지침서.md  ***REMOVED***Lite (실제 사용, 간결)
│   └── [보안-정책]_보안-워크플로우-강제.md
│
├── 📚 guides/                    ***REMOVED***✅ NEW: 안내서 폴더 (solutions → 자동화)
│   ├── README.md                 ***REMOVED***가이드 인덱스 (자동 생성)
│   ├── [가이드]개인키-관리.md
│   ├── [가이드]환경변수-설정.md
│   └── ... (2-3개 가이드만 유지)
│
├── 🗂️ archives/                  ***REMOVED***✅ NEW: 아카이브 폴더
│   ├── instructions/             ***REMOVED***지난 명령 기록 (Read-only)
│   ├── solutions/                ***REMOVED***완료된 해결 기록 (자동 정리)
│   └── reports/                  ***REMOVED***과거 분석 보고서 (매월 자동 정리)
│
└── 📂 레거시/                     ***REMOVED***구 구조 (Migration 중)
```
...
##***REMOVED***단계 2️⃣ : AI 워크플로우 우선순위 기반 재설계

**목표**: 6단계 Full → 3가지 우선순위 기반 (Lite/Mid/Full)

###***REMOVED*****Priority A: 주요 아키텍처 변경 (Full 워크플로우)**
```
적용 대상:
- 🏗️ API 엔드포인트 추가/변경
- 🔌 새 라이브러리 통합
...
##***REMOVED***단계 3️⃣ : 자동 문서 동기화 규칙 (의사 코드)
