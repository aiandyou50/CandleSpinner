***REMOVED***🔧 환경 변수 & 배포 문제 해결 가이드 (2025-10-26)
...
##***REMOVED***Step 1️⃣: 올바른 프라이빗 키 생성
...
##***REMOVED***Step 2️⃣: .env.local 업데이트
...
#***REMOVED***🚀 배포 방법 (2가지 선택)

##***REMOVED***방법 A: Blueprint를 통한 TonConnect 배포 (권장)
...
##***REMOVED***방법 B: 프라이빗 키를 사용한 직접 배포
...
##***REMOVED***Q1: "Manifest is not valid" 오류가 계속 나옵니다
...
##***REMOVED***Q2: 프라이빗 키 생성 후 지갑 주소가 다릅니다
...
##***REMOVED***Q3: 배포 후 스마트컨트랙트 주소를 어디서 확인하나요?
...
##***REMOVED***지금 바로 실행할 명령어:

```bash
cd c:\Users\x0051\Desktop\DEV\CandleSpinner\contracts

***REMOVED***1단계: 프라이빗 키 생성
npm run generate-private-key

***REMOVED***2단계: 출력된 프라이빗 키 복사 후 .env.local에 붙여넣기
***REMOVED***(수동으로 파일 편집)

***REMOVED***3단계: 환경 검증
npm run check-env

***REMOVED***4단계: 배포 (A 또는 B 선택)
***REMOVED***A: TonConnect 방식
npm run deploy
