***REMOVED***🚨 Cloudflare Pages 환경변수 설정 - 즉시 필요
...
##***REMOVED***1. **GAME_WALLET_PRIVATE_KEY** (개인키 - 128자)
...
##***REMOVED***2. **TON_RPC_API_KEY** (Ankr API 키)
```
[당신의 Ankr API 키 입력]
```
...
#***REMOVED***🔗 Cloudflare Pages 환경변수 설정 URL
```
...
##***REMOVED***wrangler.toml의 변수 vs Cloudflare 환경변수
...
##***REMOVED***"환경변수가 설정되지 않음" 오류
- Cloudflare 대시보드에서 실제로 저장되었는지 재확인
- 배포 후 2-3분 기다렸는지 확인
- 수동 배포 또는 git push로 재배포

##***REMOVED***"bad secret key size" 여전히 발생
- 개인키가 정확히 128자인지 확인
- 오타 없는지 재확인
