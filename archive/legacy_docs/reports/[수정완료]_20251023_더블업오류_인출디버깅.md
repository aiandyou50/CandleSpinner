...
##***REMOVED*****1. React Error #310 (더블업 오류) - 수정 완료**
...
##***REMOVED*****3. 인출 API 개선 (TON 수수료 진단)**

**새로 추가된 파일:**

###***REMOVED*****a) `/api/debug-private-key` (새로 생성)**
...
###***REMOVED*****b) 인출 API 개선 (`initiate-withdrawal.ts`)**
...
**개인키가 Mnemonic이면:**
```bash
***REMOVED***wallet-tools/ 폴더의 스크립트 사용
node wallet-tools/mnemonic-to-key.mjs <니모닉코드 12개 단어>

***REMOVED***결과의 "privateKey" 값을 wrangler.toml에 설정
```

##***REMOVED*****2. 게임 지갑에 TON 입금 (필수)**
...
**해결:**
```bash
***REMOVED***게임 지갑 주소에 0.5 TON 입금
***REMOVED***(TON 지갑 앱에서 수동으로 전송)
