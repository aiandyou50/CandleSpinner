...
***REMOVED***히스토리 파일 경로를 가져옵니다. 일반적으로 다음 경로에 위치합니다:
***REMOVED***$env:APPDATA\Microsoft\Windows\PowerShell\PSReadline\ConsoleHost_history.txt
...
***REMOVED***Get-History는 메모리에 로드된 현재 세션의 기록을 가져옵니다.
...
***REMOVED***히스토리가 없는지 사용자에게 바로 확인하도록 안내
