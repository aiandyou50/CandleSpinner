...
##***REMOVED***1. SetClaimable (인출 가능 금액 설정)
...
***REMOVED***2. 프론트엔드에서 인출 시도
***REMOVED***→ "Contract is paused" 오류 발생 확인

***REMOVED***3. Unpause로 복구
npx blueprint run unpauseContract --testnet
```

---

##***REMOVED***3. Unpause (정지 해제)
...
##***REMOVED***4. UpdateGameWallet (게임 지갑 변경)
...
##***REMOVED***5. UpdateOwner (관리자 권한 이전)
...
##***REMOVED***1. Game Wallet 잔액
```bash
***REMOVED***Tonkeeper 또는 Explorer에서 확인
***REMOVED***잔액 < 인출 요청 합계: 즉시 충전 필요
```
...
##***REMOVED***"Only owner can perform this action"
**원인**: Owner가 아닌 지갑으로 실행
**해결**: Owner 지갑으로 전환

##***REMOVED***"Contract is paused"
**원인**: 컨트랙트가 정지 상태
**해결**: unpauseContract.ts 실행

##***REMOVED***"Amount exceeds max single withdraw"
**원인**: 1,000,000 CSPIN 초과 설정 시도
**해결**: 금액을 1,000,000 이하로 설정

##***REMOVED***"Insufficient game wallet balance"
