***REMOVED***🎰 CSPIN Withdrawal Smart Contract
...
contracts/
├── sources/
│   ├── CSPINWithdrawalSecure.tact     ***REMOVED***수동 승인 방식
│   ├── CSPINWithdrawalAuto.tact       ***REMOVED***자동 인출 방식 ⭐ NEW
│   └── CSPINWithdrawal.tact            ***REMOVED***구 버전 (사용 안 함)
├── scripts/
│   ├── deploySecure.ts                 ***REMOVED***Secure 버전 배포
│   ├── deployAuto.ts                   ***REMOVED***Auto 버전 배포 ⭐ NEW
│   ├── setClaimable.ts                 ***REMOVED***수동 승인 (Secure 전용)
│   ├── updateContractWallet.ts         ***REMOVED***Jetton Wallet 주소 업데이트 ⭐ NEW
│   ├── pauseContract.ts                ***REMOVED***긴급 정지
│   ├── unpauseContract.ts              ***REMOVED***정지 해제
│   └── withdrawTON.ts                  ***REMOVED***TON 수수료 인출
├── frontend-poc/
│   ├── index.html                      ***REMOVED***사용자 인출 페이지
│   └── emergency-pause.html            ***REMOVED***모바일 긴급 정지 페이지
├── build/
│   ├── CSPINWithdrawalSecure/          ***REMOVED***Secure 버전 컴파일
│   └── CSPINWithdrawalAuto/            ***REMOVED***Auto 버전 컴파일 ⭐ NEW
├── wrappers/                           ***REMOVED***TypeScript 래퍼
├── tests/                              ***REMOVED***테스트 파일
├── .env                                ***REMOVED***환경 변수
├── .gitignore                          ***REMOVED***Git 보안 설정
├── README.md                           ***REMOVED***이 파일
├── ADMIN_GUIDE.md                      ***REMOVED***관리자 기능 가이드
├── SCRIPTS.md                          ***REMOVED***스크립트 사용법
├── POC_GUIDE.md                        ***REMOVED***프론트엔드 PoC 가이드
├── GAME_INTEGRATION_GUIDE.md           ***REMOVED***게임 백엔드 통합 가이드 ⭐ NEW
├── SECURITY_RECOMMENDATIONS.md         ***REMOVED***보안 권장사항
└── 부트스트랩.md                        ***REMOVED***새 세션용 컨텍스트
```
...
##***REMOVED***2️⃣ 환경 변수 설정

`.env` 파일 확인 (이미 생성됨):
```bash
***REMOVED***⚠️ 보안: 니모닉은 여기에 저장하지 마세요!
***REMOVED***스크립트 실행 시 직접 입력합니다.
...
##***REMOVED***2. 관리자 기능 (Owner 전용)

###***REMOVED*****SetClaimable** (인출 금액 설정)
```bash
...
###***REMOVED*****Pause/Unpause** (긴급 정지/재개)
...
###***REMOVED*****UpdateGameWallet** (게임 지갑 변경)
```bash
npx blueprint run setClaimable --mainnet
***REMOVED***(스크립트 내에서 UpdateGameWallet 메시지 전송)
```

게임 Jetton Wallet 주소를 변경합니다.

###***REMOVED*****WithdrawTON** (TON 수수료 인출)
```bash
...
##***REMOVED***3. Reentrancy 방지
- ✅ CEI 패턴 (Check-Effects-Interactions) 적용
- ✅ 상태 먼저 변경 → 외부 호출은 마지막

##***REMOVED***4. Access Control
- ✅ Owner 전용 관리 기능
- ✅ `requireOwner()` 검증
...
##***REMOVED***CSPINWithdrawalSecure (수동 승인)
...
##***REMOVED***CSPINWithdrawalAuto (자동 인출) ⭐ 권장
...
##***REMOVED***CSPINWithdrawalSecure (수동)
```tact
...
##***REMOVED***CSPINWithdrawalAuto (자동) ⭐
```tact
...
##***REMOVED***방법 1: Tonkeeper 앱
1. 전송(Send) → 컨트랙트 주소 입력
2. 금액: 0.05 TON
3. 메시지: `pause`
4. 전송

##***REMOVED***방법 2: 모바일 웹 페이지 (권장)
```
frontend-poc/emergency-pause.html
```
...
***REMOVED***2. 사용자에게 인출 금액 설정
npx blueprint run setClaimable --mainnet
***REMOVED***입력: 사용자 주소, 금액 (예: 100 CSPIN)

***REMOVED***3. 사용자가 프론트엔드에서 인출
***REMOVED***(frontend-poc/index.html)

***REMOVED***4. 필요 시 긴급 정지
npx blueprint run pauseContract --mainnet
***REMOVED***또는 모바일로 즉시 정지

***REMOVED***5. TON 수수료 인출
npx blueprint run withdrawTON --mainnet
***REMOVED***입력: 인출할 TON 금액
```
