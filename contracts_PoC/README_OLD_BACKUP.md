***REMOVED***🎰 CSPIN Withdrawal Smart Contract***REMOVED***CSPIN Withdrawal System - Smart Contract***REMOVED***🎯 CandleSpinner 스마트컨트랙트 프로젝트
...
#***REMOVED***📋 프로젝트 개요> 게임지갑에서 사용자 지갑으로 CSPIN 토큰을 인출하는 스마트 컨트랙트  #***REMOVED***📋 개요
...
- ✅ 보안 기능 (긴급 정지, 인출 한도)#***REMOVED***🎯 핵심 기능
...
#***REMOVED***🚀 빠른 시작- ✅ **사용자가 수수료 부담**: `forward_ton_amount` 파라미터---



##***REMOVED***1. 설치- ✅ **안전한 거래**: Owner 검증 + 유효성 검사



```bash- ✅ **추적 가능한 거래**: 모든 거래 로깅#***REMOVED***� **배포자 지갑 정보** (DO NOT LOSE)
...
##***REMOVED***2. 컴파일#***REMOVED***📁 프로젝트 구조⚠️ **중요**: 다음 정보는 절대 분실하면 안 됩니다!
...
npx blueprint run deploySecure --testnet│   └── CSPINWithdrawal.tact      ***REMOVED***메인 스마트 컨트랙트pledge relax rib furnace squirrel sugar find daughter load proof please speed
...
```bash│   └── deploy.ts                  ***REMOVED***배포 스크립트

npx blueprint run deploySecure --mainnet

```├── wrappers/##***REMOVED***생성된 주소



---│   └── CSPINWithdrawal.ts        ***REMOVED***TypeScript 래퍼- **테스트넷**: `0QB_yGkOExm0kP1--22Kx7EwllpC67Fk2xpZcfjUt7vic87g`



#***REMOVED***📂 디렉토리 구조├── build.tact                     ***REMOVED***빌드 설정- **메인넷**: `UQC2DJ8yOisLaWh7J7xHAx6yppyZCoyf5cR5vbOVJcwVQZdC`



```├── package.json                   ***REMOVED***프로젝트 설정

contracts/

├── sources/                    ***REMOVED***Tact 스마트 컨트랙트├── WITHDRAWAL_SYSTEM_DESIGN.md   ***REMOVED***전체 설계서##***REMOVED***프라이빗 키

│   ├── CSPINWithdrawal.tact           ***REMOVED***기본 버전

│   └── CSPINWithdrawalSecure.tact     ***REMOVED***보안 강화 버전 (권장)└── README.md                      ***REMOVED***이 파일- `.env.local`의 `DEPLOYER_PRIVATE_KEY`로 자동 관리됨 ✅

│

├── scripts/                    ***REMOVED***관리 스크립트```

│   ├── deploySecure.ts               ***REMOVED***컨트랙트 배포

│   ├── setClaimable.ts               ***REMOVED***인출 가능 금액 설정---

│   ├── pauseContract.ts              ***REMOVED***긴급 정지

│   ├── unpauseContract.ts            ***REMOVED***정지 해제#***REMOVED***🚀 빠른 시작

│   └── quickSetClaimable.ts          ***REMOVED***빠른 설정 (Mnemonic)

│#***REMOVED***📋 개요

├── frontend-poc/              ***REMOVED***PoC 웹 인터페이스

│   └── index.html                    ***REMOVED***사용자 인출 페이지##***REMOVED***1. 설치

│

├── wrappers/                  ***REMOVED***TypeScript 래퍼```

├── build/                     ***REMOVED***컴파일된 파일

└── tests/                     ***REMOVED***로컬 테스트```bashcontracts/
...
npm install│   └── WithdrawalManager.tact       ***REMOVED***✅ 스마트컨트랙트 (400줄)
...
- **[ADMIN_GUIDE.md](./ADMIN_GUIDE.md)**: 관리자 기능 가이드│   └── WithdrawalManager.spec.ts    ***REMOVED***✅ 테스트 정의 (11개 케이스)

- **[POC_GUIDE.md](./POC_GUIDE.md)**: 프론트엔드 PoC 가이드

- **[SECURITY_RECOMMENDATIONS.md](./SECURITY_RECOMMENDATIONS.md)**: 보안 권장사항##***REMOVED***2. 환경 설정├── wrappers/
...
#***REMOVED***🔑 주요 기능`.env.local` 파일 생성 (루트 디렉토리):├── scripts/



##***REMOVED***1. 사용자 기능│   └── deployWithdrawalManager.ts   ***REMOVED***✅ 배포 스크립트

- **ClaimRequest**: 설정된 금액만큼 CSPIN 토큰 인출

```env├── build/                           ***REMOVED***컴파일 결과 (자동 생성)

##***REMOVED***2. 관리자 기능 (Owner만 실행 가능)

- **SetClaimable**: 사용자별 인출 가능 금액 설정***REMOVED***배포자 정보├── package.json                     ***REMOVED***✅ 의존성

- **Pause/Unpause**: 긴급 정지 및 재개

- **UpdateGameWallet**: 게임 지갑 주소 변경DEPLOYER_MNEMONIC=bamboo release expand income shiver gift bounce cargo course kiss goat cram pledge relax rib furnace squirrel sugar find daughter load proof please speed├── tsconfig.json                    ***REMOVED***✅ TypeScript 설정
...
***REMOVED***게임 정보└── README.md                        ***REMOVED***✅ 이 파일
...
// sources/CSPINWithdrawalSecure.tact***REMOVED***Jetton 정보#***REMOVED***🚀 빠른 시작
...
maxSingleWithdraw: 1000       // 1회 최대 인출 한도 (CSPIN)CSPIN_GAME_JETTON_WALLET=UQBFPDdSlPgqPrn2XwhpVq0KQExN2kv83_batQ-dptaR8Mtd##***REMOVED***1️⃣ 설치
...
##***REMOVED***프론트엔드 설정```bash



```javascript##***REMOVED***3. 컴파일cd contracts
...
npm test##***REMOVED***4. 테스트넷에 배포- `@ton/ton` - TON 클라이언트
...
1. [SCRIPTS.md](./SCRIPTS.md) - 스크립트 사용 중 오류#***REMOVED***📋 환경 변수- ✅ should deploy correctly
...
#***REMOVED***📜 라이선스|------|------|------|- ✅ should handle pause correctly
...
#***REMOVED***🔗 관련 링크| `CSPIN_JETTON` | CSPIN Jetton Master 주소 | `EQBZ6nHf...` |- ✅ should reject excessive amount
...
#***REMOVED***🔄 작동 원리- ✅ should pause contract and reject requests

- ✅ should accumulate multiple withdrawals

```mermaid

사용자 (프론트엔드)##***REMOVED***3️⃣ 테스트넷 배포
...
- 안전값: 0.25 TON#***REMOVED***📝 스마트컨트랙트 기능
...
##***REMOVED***인출 요청 처리   - 게임 백엔드에서 인출 요청 수신
...
fun requestWithdrawal(```tact
    ...
    send(SendParameters {}
        ...
        mode: SendPayGasSeparately,##***REMOVED***Getter 함수
...
#***REMOVED***🧪 테스트##***REMOVED***환경 변수 설정 (wrangler.toml)
...
#***REMOVED***📖 자세한 정보  CSPIN_JETTON = "EQBvp...",        ***REMOVED***CSPIN Jetton Master
...
- **구현 방식**: 스마트 컨트랙트, 백엔드, 프론트엔드##***REMOVED***TypeScript에서 사용
