***REMOVED***💡 CSPIN 충전 및 회수 가이드

#***REMOVED***질문 답변

##***REMOVED***1. 인출하려면 EQBsRMhs7iA5TazpkTInpVv5kb6hqMCVzWcpG3hv1lvD6gGa 주소로 CSPIN을 전송해야 하나요?
...
##***REMOVED***3. 컨트랙트에 있는 CSPIN을 Owner가 회수할 수 있나요?

**답변: Yes! ✅ 방금 추가했습니다!**

###***REMOVED***새로 추가된 기능: WithdrawJetton
...
**PowerShell 스크립트 (고급)**:
```powershell
***REMOVED***Owner Jetton Wallet → Contract Jetton Wallet
***REMOVED***스크립트는 아직 없음, Tonkeeper 사용 권장
```
...
##***REMOVED***STEP 2: CSPIN 회수 (Owner 전용)

###***REMOVED***PowerShell에서 실행:
...
```powershell
***REMOVED***Jetton Wallet 주소 계산
npx ts-node scripts/getJettonWalletAddress.ts

***REMOVED***CSPIN 회수 (Owner → 컨트랙트)
npx ts-node scripts/withdrawJetton.ts
